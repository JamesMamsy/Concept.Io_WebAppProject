...
import django
django.setup()
from conceptio.models import Category, Project, UserProfile
...
def populate():
    ...
    for cat, cat_data in categories.items():
        ...
        for p in cat_data['projects']:
            add_project(c, users[p['creator']], p['title'], p['desc'], p['tags'], p['likes'])

    for c in Category.objects.all():
        for p in Project.objects.filter(cat = c):
            print(f'- {c}: {p}')
...
def add_project(category,creator,title,desc,tags,likes):
    p = Project.objects.get_or_create(cat= category.name, title=title, creator=creator)[0]
    p.desc = desc
    p.tags = tags
    # p.likes = likes
    p.dislikes = likes
