...
def populate():
     ...
     for cat, cat_data in categories.items():
         ...
         for p in cat_data['projects']:
             add_project(c, p['title'], p['desc'],p['cat'],p['tags'],p['likes'],p['dislikes'])
...
def add_project(cat, title, desc,category,tags,likes,dislikes, views=0):
     p = Project.objects.get_or_create(cat=cat, title=title)[0]
     p.desc=desc
     p.cat=category
     p.tags=tags
     p.likes=likes
